...
```mermaid
graph LR
  classNames(....) --> [Step 1: Create account]
  classNames(....) --> [Step 2: Profile information]
  classNames(....) --> [Step 3: Business information]
  classNames(....) --> [Step 4: Theme]
  classNames(....) --> [Step 5: Preview]
steps:
  [Step 1: Create account] --> [current]
  [Step 2: Profile information] --> [current]
  [Step 3: Business information] --> [current]
  [Step 4: Theme] --> [current]
  [Step 5: Preview] --> [current]

```
This overview uses the `graph` and `steps` keywords to define the structure of the navigation menu and the steps in the process, respectively. The `classNames` function is used to generate the class names for each step, based on the `id` of the step. The `aria-current` attribute is used to set the current step
