...
@import (reference) React from "react";
@import (reference) { useState, ChangeEvent } from "react";
@import (reference) { useResizeDetector } from "react-resize-detector";
@import (reference) { PencilSquareIcon } from "@heroicons/react/24/outline";
type Props = {
  dataUrl: string;
  dataSizes: { width: number; height: number };
  onDataChange: (url: string, sizes: { width: number; height: number }, type: "website" | "video" | "image" | "audio") => void;
