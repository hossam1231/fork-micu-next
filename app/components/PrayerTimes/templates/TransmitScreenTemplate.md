...
function TinyWaveFormIcon({ colors = [], ...props }) {
  return (
    <svg aria-hidden="true" viewBox="0 0 10 10" {...props}>
      <path d="M0 5a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v4a1 1 0 0 1-1 1H1a1 1 0 0 1-1-1V5Z" className={colors[0]} />
      <path d="M6 1a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v8a1 1 0 0 1-1 1H7a1 1 0 0 1-1-1V1Z" className={colors[1]} />
    </svg>
  );
}
...
function AboutSection({ subMessage, ...props }) {
  ...
  return (
    <section {...props}>
      <h2 style={{ color: props?.theme?.timeCardTextColor }} className="flex items-center font-mono text-sm font-medium leading-7">
        <TinyWaveFormIcon colors={["fill-violet-300", "fill-pink-300"]} className="h-2.5 w-2.5" />
        <span style={{ color: props?.theme?.timeCardTextColor }} className="ml-2.5">
          Attention
        </span>
      </h2>
      <p
      ...
      {props.showSubMessageEditor && (
        <div className="absolute left-0 top-[350px] z-50 m-4 h-fit  w-[95%] rounded-lg bg-white shadow-lg" onClick={(e) => e.stopPropagation()}>
          <Editor
...
export default function TransmitScreenTemplate({
  ...
  let hosts = [currentEstablishment?.name || "", currentEstablishment?.zip || ""];
    ...
    >
      ...
      >
        <div className="hidden lg:sticky lg:top-0 lg:flex lg:w-16 lg:flex-none lg:items-center lg:whitespace-nowrap lg:py-12 lg:text-sm lg:leading-7 lg:[writing-mode:vertical-rl]">
          ...
          <span className="mt-6 flex gap-6 font-bold " style={{ color: theme?.textColor }}>
        ...
        <div className="relative z-10 mx-auto px-4 pb-4 pt-10 sm:px-6 md:max-w-2xl md:px-4 lg:min-h-full lg:flex-auto lg:border-x lg:border-slate-200 lg:px-8 lg:py-12 xl:px-12">
          ...
          {showTopMessageEditor && (
            <div className="top-[20 0px] absolute  left-0 z-50 m-4 h-fit  w-[95%] rounded-lg bg-white shadow-lg" onClick={(e) => e.stopPropagation()}>
              <Editor
                title="Edit Top Message"
                value={topMessage}
                onChange={(content) => {
            ...
            <ul role="list" className="mt-4 flex justify-center gap-10 text-base font-medium leading-7 text-slate-700 sm:gap-8 lg:flex-col lg:gap-4">
              {[
                ...
                <li key={label} className="flex">
                  <a href="/" className="group flex items-center" aria-label={label}>
      ...
      <main className="lg:ml-112 xl:ml-120 border-t border-slate-200 lg:relative lg:mb-28 lg:border-t-0">
        ...
        <div className="relative left-[40%] w-[55%]">
          <div className="relative top-2">
            ...
            >
              <h1 className="mb-1 mt-2 text-center font-bold  " style={{ fontSize: "2.4vw" }}>
                <span>{currentTime}</span>
              </h1>
              <p className="f mb-4 text-center" style={{ color: theme.timeCardTextColor, fontSize: "1.5vw" }}>
      ...
      <div className="lg:left-112 xl:left-120 fixed inset-x-0 bottom-0 z-10">{/* <AudioPlayer /> */}</div>
    </div>
  );
}
...
.008 24.8 27.528 24.8 27.528 24.8Z" />
