...
function Prayers({ prayers = [], refetch, page = 1, publicEstablishmentId = "", total = 0, currentEstablishment }: Props) {
  const router = useRouter();

  const [curPrayers, setCurPrayers] = React.useState<Prayer[] | null>(null);
  ...
  const navigateTo = (id: string) => {
    router.push(`/dashboard/prayer/${publicEstablishmentId}?id=${id}`);
  };
  ...
  return (
    <>
      <RecordViewer
        ...
        onEndReached={() => {
          if (curPrayers && curPrayers.length < total) {
            refetch(page + 1);
          }
        }}
      />
      {showAddScreen && <AddPrayer setCurPrayers={setCurPrayers} publicEstablishmentId={publicEstablishmentId} close={() => setShowAddScreen(false)} />}
    </>
  );
}
