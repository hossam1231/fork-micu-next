...
function TimesImporter({ importData, setTableData, close }: Props) {
  const [model, setModel] = React.useState(defaultModel) as any;
  ...
  const importPrayerTimes = () => {
    const newParayerTimes = [] as any;
    importData.data.forEach((row: { [x: string]: string | number | Date }, i: string | number) => {
      if (i !== 0) {
        const newPrayerTime = {} as any;
        fields.forEach((field) => {
          //@ts-ignore
        ...
        {fields.map((field) => (
          <div className="mb-2 mr-4 flex w-[45%] flex-col">
            ...
            >
              ...
              {importData?.headers?.map((header: any) => (
