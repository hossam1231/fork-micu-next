...
function PrayerOptionsModal({ setShowPrayerTimesEditor, prayerTimes, setPrayerTimes }: Props) {
  ...
  React.useEffect(() => {
    ...
    const columns = prayerFields.map((col) => ({
      ...
      ) => React.useCallback(<Cell row={row} val={row[col.key]} prop={col.key} setTableData={setPrayerTimes} />, [row, index]),
    }));
...
const Cell = ({ row, val, prop, index, setTableData, tableData }: CellProps) => {
    ...
    <input
      ...
      onBlur={(e) => {
        ...
        setTableData((prev: any[]) => {
          const findIndex = prev.findIndex((data: { id: any }) => data.id === row.id);
