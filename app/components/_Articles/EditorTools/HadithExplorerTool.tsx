import { render, unmountComponentAtNode } from "react-dom";
import { setNewRootId } from "@/app/_helpers/web/formatters";
import SelectedHadith from "../../Isnad/SelectedHadith";

export class HadithExplorerTool {
  api: any;
  readOnly: any;
  data: any;
  CSS: { wrapper: string };
  nodes: { holder: null };
  config: any;
  root: any;
  editor: any;
  constructor({ data, config, api, readOnly, editor }) {
    this.api = api;
    this.readOnly = readOnly;
    this.data = data;
    this.config = config;
    this.editor;
    this.root = null;

    this.CSS = {
      wrapper: "walkthrough-timeline",
    };

    this.nodes = {
      holder: null,
    };

    console.log("data", data, config, api, readOnly);
  }

  static get toolbox() {
    return {
      icon: `<img
          width="16px"
          src="data:image/png;base64,"
        ></img>`,
      title: "Hadith Explorer",
    };
  }

  static get isReadOnlySupported() {
    return true;
  }

  insertVerse = (verses) => {
    //get block index to insert 1 block before it
    const index = this.api.blocks.getCurrentBlockIndex();
    //insert block before current block

    return this.api.blocks.insert(
      "quote",
      {
        text: verses,
      },
      {},
      index
    );
  };

  render() {
    const rootNode = document.createElement("div");
    rootNode.setAttribute("class", this.CSS.wrapper);
    //@ts-expect-error
    this.nodes.holder = rootNode;

    //render when  only in viewport

    if (!this.data.id) {
      this.data = {
        id: "hadithexplorer_" + Date.now(),
        selectedVerses: [],
      };
    }

    const onDataChange = (verses) => {
      this.data = { ...this.data, selectedVerses: verses };
    };

    this.root = rootNode;

    setNewRootId(this.root);

    render(
      <SelectedHadith verses={this.data.selectedVerses} onDataChange={onDataChange} readOnly={this.readOnly} insertVerses={this.insertVerse} />,
      this.root
    );

    return this.nodes.holder;
  }

  destroy() {
    unmountComponentAtNode(this.root);

    console.log("destroy");
  }

  save() {
    return this.data;
  }
}
