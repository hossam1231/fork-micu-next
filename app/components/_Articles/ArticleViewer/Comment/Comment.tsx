...
const Comment = ({ index, text, replies, reactions, onReply, onReaction, isReply, replyindex }: Props) => {
  const [newReply, setNewReply] = useState("");
  const [replying, setReplying] = useState(false);

  const handleReplyChange = (text: SetStateAction<string>) => {
    setNewReply(text);
  };
    ...
    <div className="mb-4">
      <div className="mb-2 flex">
        ...
        <div className="flex-grow">
          <div className="mb-2 flex flex-row">
            <img className="h-6 w-6 rounded-full" src="https://i.pravatar.cc/100?img=2" alt="User Avatar" />
          ...
          <div className="flex items-center justify-end">
            <button className="mr-2 font-medium text-gray-500 hover:text-gray-800" onClick={handleReactionClick}>
              {reactions !== 0 ? (
                <>
                  <svg className="mr-1 inline-block h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                    <path
                      fill="currentColor"
            ...
            {!isReply && (
              <button className="text-xs font-medium text-gray-500 hover:text-black hover:text-gray-800" onClick={() => setReplying(!replying)}>
                Reply
              </button>
            )}
          </div>
      ...
      {replying && (
        <>
          <div className="mb-2 flex">
            <div className="mr-3 flex-shrink-0">
              <img className="h-8 w-8 rounded-full" src="https://i.pravatar.cc/100?img=3" alt="User Avatar" />
            </div>
            <div className="flex-grow">
