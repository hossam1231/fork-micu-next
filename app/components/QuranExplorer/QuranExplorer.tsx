...
function QuranExplorer({ close, addVerse, presetIndex, readOnly = false }: Props) {
  ...
  return (
    <>
      <div style={{ width: "90vw" }} className="modal-center modal-center-large  h-[94vh] p-8 ">
                ...
                <div className="hide-scrollbar mt-2 flex max-h-[25vh] flex-row-reverse flex-wrap overflow-y-scroll p-0.5">
                  {ayah?.verses &&
                    Object.keys(ayah?.verses).map((key) => {
                      ...
                      return verse?.words?.map((word) => <QuranWord key={word?.id} word={word} setAudioUrl={setAudioUrl} setSelectedAyah={undefined} />);
                    })}
                </div>
      ...
      <Popover plusY={-20} event={tafsirsPopoverEvent} show={tafsirsPopoverEvent !== null} onClose={() => setTafsirsPopoverEvent(null)} placement={undefined}>
