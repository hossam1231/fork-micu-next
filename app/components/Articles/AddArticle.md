...
function AddArticle({ close, setCurArticles, publicEstablishmentId }: Props) {
  ...
  const onSubmit = async (data: { content: string; establishmentPublicId: string }) => {
    ...
    requestHandler({ type: "post", body: data, route: "articles" }).then((res) => {
      setLoading(false);
      ...
      if (!res?.errors) {
        ...
        setCurArticles((prev) => [...prev, curData]);
    ...
    <>
      <div className="modal-center">
        <div className="modal-center-medium h-[95vh] overflow-hidden rounded-md bg-white p-8 shadow-xl">
          <h2 className="text-indigo-600">Add Article</h2>
