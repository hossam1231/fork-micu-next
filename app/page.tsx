...
const checkRootProcess = async () => {
  ...
  if (subdomain === "admin") {
    ...
    const [site] = (await (
      await conn
    ).query("SELECT id, description, label, permissions, adminIds, siteData, userId, thumbnail, createdAt, updatedAt , siteData FROM Site WHERE label = ?", [
      subdomain,
    ])) as any;

    if (!site || site.length === 0) return null;

    const [SitesPage] = await (await conn).query("SELECT id, description, content, label FROM SitePage WHERE siteName = ? AND label = ?", [subdomain, "home"]);
...
export async function generateMetadata({ params }: Props, parent: ResolvingMetadata): Promise<Metadata> {
  ...
  return {
    title: params[params.length - 1] || "Home",
    description: "test",
  };
}
