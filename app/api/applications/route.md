...
graph LR
    ...
    | User |
    +-------------------------------+
    | Email | Telephone | First Line | Second Line | Zip | Country | Building Number | City | County | Institution Name | Type |
...
```
This overview shows the different endpoints in your Go file, along with their corresponding HTTP methods and the parameters they accept. The endpoints are represented as nodes in the graph, and the parameters are represented as edges between the nodes. The graph is organized into categories based on the HTTP methods (GET, POST, PUT, DELETE) and the types of data they handle (User, Email, Telephone, etc.).
