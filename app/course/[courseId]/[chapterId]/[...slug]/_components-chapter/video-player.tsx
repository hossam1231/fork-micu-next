...
export const VideoPlayer = ({ playbackId, courseId, chapterId, nextChapterId, isLocked, completeOnEnd, title }: VideoPlayerProps) => {
  ...
  const onEnd = async () => {
    try {
      if (completeOnEnd) {
        ...
        await requestHandler({ type: 'put', url: `courses/${courseId}/chapters/${chapterId}/progress`, body: { isCompleted: true } });
  ...
  return (
    <div className="relative aspect-video">
      ...
      {!isLocked && (
        //We could just use the default video player, and set up any additional buttons we need , pricing may become an issue after for mux
