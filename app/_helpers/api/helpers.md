...
* `body`: The request body to be validated.
* `schema`: The validation schema.
* `session`: The session object, used to check if the request is authenticated.
* `db`: The database connection, used to execute the validation queries.

#### Method

Returns a validation error message if the request is invalid, or `null` if the request is valid.
...
* `connection`: The Knex connection to use for the query.

#### Method

Returns a session object if the request is authenticated, or `null` if the request is not authenticated.
...
Calls the `CheckErrors` method with the request body and validation schema, and returns a validation error message if the request is invalid, or `null` if the request is valid.
