...
* `setCache(key: string | number, value: any, options?: CacheOptions)`: Sets a value in the cache with the given key and value, and options.
* `addServerCacheValue(value: any, key?: string | number, options?: CacheOptions)`: Adds a value to the server cache with the given key and value, and options.
* `bulkAddServerCacheValue(cacheArray: CacheArray)`: Adds multiple values to the server cache at once.
* `getServerCacheValue(key: string | number, options?: CacheOptions)`: Returns the value stored in the cache with the given key, or `null` if it doesn't exist.
* `bulkGetServerCacheValue(CacheArray: CacheArray)`: Returns an array of values stored in the cache.
* `deleteFromServerCache(key: string | number, options?: CacheOptions)`: Deletes the value stored in the cache with the given key, or `null` if it doesn
