...
```mermaid
graph LR
    + UseEstablishment[useEstablishment] --> + CheckAuthirization[checkAuthirization]
    + CheckAuthirization[checkAuthirization] --> + GetCachData[getCachData]
    + GetCachData[getCachData] --> + HadleEstablishmentLogin[handleEstablishmentLogin]
    + HadleEstablishmentLogin[handleEstablishmentLogin] --> + AddToCache[addToCache]
    + AddToCache[addToCache] --> + UseRouter[useRouter]
    + UseRouter[useRouter] --> + Navigate[navigate]
    + Navigate[navigate] --> + Render[render]
    + Render[render] --> + Return[return]
    + Return[return] --> + UseEffect[useEffect]
    + UseEffect[useEffect] --> + Return[return]

```
This graph shows the dependencies between the functions and how they interact
