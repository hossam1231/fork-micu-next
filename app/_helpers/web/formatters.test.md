...
* `import { render, waitFor, screen } from '@redwoodjs/testing/web'`
* `import { formatEnum, jsonTruncate, truncate, timeTag, jsonDisplay } from './formatters'`

### Formatters

* `formatEnum`: Tests the `formatEnum` function with various input types and edge cases.
* `truncate`: Tests the `truncate` function with various input types and lengths.
* `timeTag`: Tests the `timeTag` function with various input types and formats.
* `jsonDisplay`: Tests the `jsonDisplay` function with various input types and formats.

### Description

* `describe`: Runs a series of tests for the `formatEnum`, `truncate`, `timeTag`, and `jsonDisplay` functions.



```

This overview provides a high-level view of the tests and their expected outputs, which can be used to quickly understand the functionality of the `formatters` package in Go.
