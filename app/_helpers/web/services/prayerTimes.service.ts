...
const prayerTimes = [
  ...
  {
    date: new Date(new Date().getTime() + 48 * 60 * 60 * 1000).toLocaleDateString(),
...
const checkNextPrayer = (
  ...
  if (currentTimeHour < fajrTimeHour || (currentTimeHour == fajrTimeHour && currentTimeMinute < fajrTimeMinute)) {
    console.log("runs fajr");
    setNextPrayer({
      name: "Fajr",
      time: prayerTime.fajr_jamaat,
    });
  } else if (currentTimeHour < dhuhrTimeHour || (currentTimeHour == dhuhrTimeHour && currentTimeMinute < dhuhrTimeMinute)) {
    setNextPrayer({
      name: "Dhuhr",
      time: prayerTime.dhuhr_jamaat,
    });
  } else if (currentTimeHour < asrTimeHour || (currentTimeHour == asrTimeHour && currentTimeMinute < asrTimeMinute)) {
    setNextPrayer({
      name: "Asr",
      time: prayerTime.asr_jamaat,
    });
  } else if (currentTimeHour < maghribTimeHour || (currentTimeHour == maghribTimeHour && currentTimeMinute < maghribTimeMinute)) {
    setNextPrayer({
      name: "Maghrib",
      time: prayerTime.maghrib_jamaat,
    });
  } else if (currentTimeHour < ishaTimeHour || (currentTimeHour == ishaTimeHour && currentTimeMinute < ishaTimeMinute)) {
    console.log("runs");
    setNextPrayer({
      name: "Isha",
