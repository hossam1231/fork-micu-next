  ...
  return (
    ...
    >
      <div className="flex items-center gap-x-2 py-4">
        <Icon size={22} className={`text-slate-500 ${isActive ? "text-slate-700" : ""} ${isCompleted && "text-emerald-700"}`} />
        {label}
      </div>
      <div
        className={`ml-auto opacity-0 border-2 border-slate-700 h-full transition-all ${isActive ? "opacity-100" : ""} ${
