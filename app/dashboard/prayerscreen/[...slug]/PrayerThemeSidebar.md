...
class PrayerThemeSidebar {
  ...
  // Methods
  @method setTheme(theme: { backgroundColor: string; textColor: string; highlightColor: string; timeCardColor: string; timeCardTextColor: string; fontFamily: string; template: string; }) {
    this.theme = theme;
  }
  @method close() {
    // Close the sidebar
...
Here is the generated Mermaid Markdown code for the Go file you provided:
```mermaid
graph LR
    A[PrayerThemeSidebar] --> B[Theme Settings]
    B --> C[Background Color]
    C --> D[Value: {theme.backgroundColor}]
    B --> E[Text Color]
    E --> F[Value: {theme.text
