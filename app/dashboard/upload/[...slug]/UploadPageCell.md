...
export default UploadPageCellEachLayout;
```
Generate the Mermaid diagram for the above Go file by running the following command:
```bash
$ mermaid --config=./mermaid.config.js --output=pdf --output=png --output=svg UploadPageCellEachLayout.go
```
This will generate a PDF, PNG, and SVG diagram of the Go file. You can customize the diagram by modifying the `mermaid.config.js` file to include additional configuration options.
